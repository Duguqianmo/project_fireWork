...
function init() {
	...
	if (startButtonContainer && startButton) {
		...
		startButton.addEventListener('click', function (e) {
			...
			// 转换为舞台坐标（如果stageW和stageH已定义）
			let stageX = buttonX;
			let stageY = buttonY;
			if (typeof stageW !== 'undefined' && typeof stageH !== 'undefined') {
				// 计算相对于canvas的坐标
				const canvasRect = appNodes.stageContainer.getBoundingClientRect();
				stageX = buttonX - canvasRect.left;
				stageY = buttonY - canvasRect.top;
			}
			...
			// 添加黑洞收缩动画效果
			startButton.style.animation = 'button-burst 0.6s cubic-bezier(0.5, 0, 0.8, 1) forwards';
			startButton.style.transition = 'all 0.6s cubic-bezier(0.5, 0, 0.8, 1)';

			// 添加强烈的发光效果，模拟黑洞吸积盘
			startButton.style.boxShadow = '0 0 80px rgba(0, 255, 255, 1), 0 0 160px rgba(0, 255, 255, 0.9), 0 0 240px rgba(128, 0, 255, 0.8), inset 0 0 40px rgba(0, 255, 255, 0.5)';

			// 延迟隐藏按钮，让黑洞收缩动画和烟花效果先显示
			setTimeout(() => {
